...
void RenderWorker::requestPage(quint64 requestId, int pageNumber, QSize imageSize, QPdfDocumentRenderOptions options) {
    const QMutexLocker locker(&m_mutex);

    if ( !m_document || m_document->status() != QPdfDocument::Ready )
        return;

    const QImage image = m_document->renderPage( pageNumber, imageSize, options );
