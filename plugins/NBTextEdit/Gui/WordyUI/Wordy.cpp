...
Wordy::Wordy( QString fn, QWidget *parent ) : QMainWindow( parent ) {
...
void Wordy::setWindowProperties() {
...
void Wordy::fileProps() {

	wFileProps *props = new wFileProps( ed );
	props->exec();
};

void Wordy::changeFont( QFont newFont ) {

	QSettings settings( "NewBreeze", "WordyPlugin" );
	settings.setValue( "Font", newFont );

	ed->setFont( newFont );
};

void Wordy::updateModStatus( bool modified ) {

	safetyLed->setSafe( not modified );
};

void Wordy::updateStatusBar() {
...
void Wordy::resizeEvent( QResizeEvent *rEvent ) {
...
void Wordy::closeEvent( QCloseEvent *cEvent ) {
