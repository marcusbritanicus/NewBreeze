...
void UDisks2::addBlock( const QString& node ) {
    if ( blocks_.contains( node ) ) {
        ...
        connect( block, &UDisks2Block::filesystemAdded,   this, &UDisks2::filesystemAdded );
        connect( block, &UDisks2Block::filesystemRemoved, this, &UDisks2::filesystemRemoved );
        connect( block, &UDisks2Block::filesystemChanged, this, &UDisks2::filesystemChanged );
        connect( block, &UDisks2Block::changed,           this, &UDisks2::blockDeviceChanged );
...
UDisks2Block::UDisks2Block( const QString& node, QObject *parent ) :
    ...
    system.connect(
        ...
        "PropertiesChanged",
        this,
        SLOT( self_propertiesChanged( QString, QVariantMap, QStringList ) )
    );
    update();
...
QString UDisks2Block::toString() {
    return QString(
        ...
        "usage: %7\n"
        "type: %8"
    ).arg( name ).arg( dev ).arg( id ).arg( drive ).arg( size ).arg( readonly ).arg( usage ).arg( type );
}
