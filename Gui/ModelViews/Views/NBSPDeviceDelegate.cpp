/*
	*
	* NBSPDeviceDelegate.cpp - NewBreeze ItemDelegate Class
	*
*/
...
void NBSPDeviceDelegate::paint( QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index ) const {

	if ( index.column() == 0 ) {
		painter->save();
		const NBDeviceModel *model = static_cast<const NBDeviceModel*>( index.model() );
...
QSize NBSPDeviceDelegate::sizeHint( const QStyleOptionViewItem &optItem, const QModelIndex &mIndex ) const {
