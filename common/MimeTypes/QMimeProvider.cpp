...
#include "QMimeProvider_p.hpp"

#ifndef QT_NO_MIMETYPE

#include "QMimeTypeParser_p.hpp"
#include "QStandardPaths.hpp"
#include "QMimeMagicRuleMatcher_p.hpp"
...
NBCOMMON_DLLSPEC int qmime_secondsBetweenChecks = 5; // exported for the unit test
...
void QMimeBinaryProvider::checkCache()
{
    ...
    if (cacheFileNames != m_cacheFileNames) {
        Q_FOREACH (const QString &cacheFileName, cacheFileNames) {
...
QStringList QMimeBinaryProvider::findByFileName(const QString &fileName, QString *foundSuffix)
{
    ...
    Q_FOREACH (CacheFile *cacheFile, m_cacheFiles) {
...
void QMimeBinaryProvider::loadMimeTypeList()
{
    if (!m_mimetypeListLoaded) {
        ...
        Q_FOREACH (const QString &typeFilename, typesFilenames) {
...
QMimeType QMimeXMLProvider::findByMagic(const QByteArray &data, int *accuracyPtr)
{
    ...
    Q_FOREACH (const QMimeMagicRuleMatcher &matcher, m_magicMatchers) {
...
void QMimeXMLProvider::ensureLoaded()
{
    if (!m_loaded || shouldCheck()) {
        ...
        Q_FOREACH (const QString &packageDir, packageDirs) {
            QDir dir(packageDir);
