...
template <typename T>
inline T qUnalignedLoad(const void *ptr)
{
    T result;
      ...
      m_matchFunction(0)
{
    ...
    const QString startPosStr = offsets.midRef(0, colonIndex).toString(); // \ These decay to returning 'offsets'
    const QString endPosStr   = offsets.midRef(colonIndex + 1).toString();// / unchanged when colonIndex == -1
