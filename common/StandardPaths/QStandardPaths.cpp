                                       ...
                                       const QString &executableName)
{
    ...
    Q_FOREACH (const QString &searchPath, searchPaths) {
...
QString QStandardPaths::findExecutable(const QString &executableName, const QStringList &paths)
{
    ...
    if (paths.isEmpty()) {
        ...
        const QStringList rawPaths = QString::fromLocal8Bit(pEnv.constData()).split(":", QString::SkipEmptyParts);
        searchPaths.reserve(rawPaths.size());
        Q_FOREACH (const QString &rawPath, rawPaths) {
